...
for r in responses:
    if not os.path.exists(folder+str(r["session"])):
...
plt.pie(counts, labels=labels, colors=colors,
        autopct=lambda(p): '{:.0f}'.format(p * total / 100), shadow=False, startangle=90, explode = (0.05, 0))
